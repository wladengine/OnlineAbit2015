    ...
    public class XLSUtils
    {
        public static byte[] PrintAllToExcel2007(GlobalCommunicationModelApplicantList model, string sheetName, string fileName)
        {
            ...
            using (ExcelPackage doc = new ExcelPackage(newFile))
            {
                ...
                List<string> ColNames = new List<string>() { "Number", "Applicant", "in Roman Letters", "Is complete", "Portfolio Ru", "Portfolio De", "Portfolio Common",
                ...
                foreach (var dc in model.ApplicantList)
                {
                    ...
                    ws.Cells[i, 4].Value = dc.isComplete? "yes" : "no";
                    ws.Cells[i, 5].Value = dc.PortfolioAssessmentRu;
                    ws.Cells[i, 6].Value = dc.PortfolioAssessmentDe;
                    ws.Cells[i, 7].Value = dc.PortfolioAssessmentCommon;
                    ws.Cells[i, 8].Value = dc.Interview ? "yes" : "no";
                    ws.Cells[i, 9].Value = dc.InterviewAssessmentRu;
                    ws.Cells[i, 10].Value = dc.PortfolioAssessmentDe;
                    ws.Cells[i, 11].Value = dc.PortfolioAssessmentCommon;
                    ws.Cells[i, 12].Value = dc.OverallResults;
                    i++;
                }
