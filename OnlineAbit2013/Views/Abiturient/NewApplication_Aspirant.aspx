...
<% using (Html.BeginForm("NewApp_Asp", "Abiturient", FormMethod.Post))
   { 
%> 
    <% if (Model.HasError)
       { %>
    ...
    <% if (!String.IsNullOrEmpty(Model.OldCommitId)){ %><%= Html.HiddenFor(x => x.OldCommitId)%><%} %>
        ...
        <div id="Block<%= i.ToString()%>" class="message info panel" style="width:659px; display:none;">
            ...
            <div id="IsCrimea<%= i.ToString()%>" style="display:none;" >
                 ...
                 <input type="hidden" name="isForeignHidden" title="Поступать по конкурсу для лиц, постоянно проживающих в Крыму" id="isCrimeaHidden<%= i.ToString()%>" /> 
            </div>
